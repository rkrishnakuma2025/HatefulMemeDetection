
import os
import time

import torch
import torch.nn as nn
import torch.optim as optim
from torch.utils.data import DataLoader
from torchvision import models

from dataset import HatefulMemesDataset, train_transform, eval_transform


# === CONFIG ===
DATA_ROOT = r"C:\Users\resmi\Downloads\Hateful_Meme_Detection\meme_robustness_data\data" 
BATCH_SIZE = 32
NUM_EPOCHS = 8
LR = 1e-4
NUM_WORKERS = 2  
OUTPUT_DIR = "outputs_baseline"


def create_dataloaders(): # create the dataloaders for the training and development sets
    train_ds = HatefulMemesDataset(DATA_ROOT, split="train", transform=train_transform)
    dev_ds = HatefulMemesDataset(DATA_ROOT, split="dev", transform=eval_transform)

    train_loader = DataLoader(
        train_ds, batch_size=BATCH_SIZE, shuffle=True,
        num_workers=NUM_WORKERS, pin_memory=True
    )
    dev_loader = DataLoader(
        dev_ds, batch_size=BATCH_SIZE, shuffle=False,
        num_workers=NUM_WORKERS, pin_memory=True
    )
    return train_loader, dev_loader


def create_model(num_classes=2):
    # ResNet18 pretrained on ImageNet
    model = models.resnet18(weights=models.ResNet18_Weights.IMAGENET1K_V1)
    # Replace final FC layer
    in_features = model.fc.in_features
    model.fc = nn.Linear(in_features, num_classes)
    return model


def evaluate(model, data_loader, device): 
    model.eval()
    correct = 0
    total = 0
    running_loss = 0.0
    criterion = nn.CrossEntropyLoss()

    with torch.no_grad(): 
        for images, labels, ids in data_loader:
            images = images.to(device)
            labels = labels.to(device)

            outputs = model(images)
            loss = criterion(outputs, labels)
            running_loss += loss.item() * images.size(0)

            _, preds = torch.max(outputs, dim=1)
            correct += (preds == labels).sum().item()
            total += labels.size(0)

    avg_loss = running_loss / total # calculate the average loss
    accuracy = correct / total # calculate the accuracy
    return avg_loss, accuracy


def train(): # train the model
    os.makedirs(OUTPUT_DIR, exist_ok=True)

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Using device: {device}")

    train_loader, dev_loader = create_dataloaders()

    model = create_model(num_classes=2)
    model = model.to(device)

    criterion = nn.CrossEntropyLoss() # create the loss function
    optimizer = optim.Adam(model.parameters(), lr=LR) # create the optimizer

    best_dev_acc = 0.0

    for epoch in range(1, NUM_EPOCHS + 1): # train the model for the number of epochs
        model.train()
        running_loss = 0.0
        running_corrects = 0
        total_samples = 0
        start_time = time.time()

        for batch_idx, (images, labels, ids) in enumerate(train_loader): 
            images = images.to(device)
            labels = labels.to(device)

            optimizer.zero_grad()

            outputs = model(images)
            loss = criterion(outputs, labels)
            loss.backward()
            optimizer.step()

            running_loss += loss.item() * images.size(0)
            _, preds = torch.max(outputs, dim=1)
            running_corrects += (preds == labels).sum().item() # calculate the number of correct predictions
            total_samples += labels.size(0)

            if (batch_idx + 1) % 50 == 0: # print the loss and accuracy every 50 batches
                avg_batch_loss = running_loss / total_samples
                avg_batch_acc = running_corrects / total_samples
                print(
                    f"Epoch [{epoch}/{NUM_EPOCHS}] "
                    f"Step [{batch_idx+1}/{len(train_loader)}] "
                    f"Train Loss: {avg_batch_loss:.4f} "
                    f"Train Acc: {avg_batch_acc:.4f}"
                )

        epoch_loss = running_loss / total_samples
        epoch_acc = running_corrects / total_samples
        elapsed = time.time() - start_time

        dev_loss, dev_acc = evaluate(model, dev_loader, device)

        print(
            f"Epoch [{epoch}/{NUM_EPOCHS}] "
            f"Train Loss: {epoch_loss:.4f}, Train Acc: {epoch_acc:.4f} | "
            f"Dev Loss: {dev_loss:.4f}, Dev Acc: {dev_acc:.4f} | "
            f"Time: {elapsed:.1f}s"
        )

        # Save best model by dev accuracy
        if dev_acc > best_dev_acc:
            best_dev_acc = dev_acc
            save_path = os.path.join(OUTPUT_DIR, "baseline_resnet18_best.pth")
            torch.save(model.state_dict(), save_path)
            print(f"  >> New best dev acc: {best_dev_acc:.4f}. Saved model to {save_path}")

    print(f"Training finished. Best dev accuracy: {best_dev_acc:.4f}")


if __name__ == "__main__":
    train()
